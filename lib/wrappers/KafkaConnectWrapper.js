...
class KafkaConnectWrapper {
    ...
    makeRequest(params, data) {
        return new Promise((resolve, reject) => {
            const req = http.request({
                ...
                res.on('end', () => {
                    try {
                        ...
                        this._logger.error('Error occurred while parsing request body', {
                            method: 'KafkaConnectManager.makeRequest',
                            error: e.message,
                            params,
                        });
                        return reject(errors.InternalError.customizeDescription(
                            `Error occurred while parsing request body, ${e.message}`));
                    }
                    ...
                    if (res.statusCode < 200 || res.statusCode >= 300) {
                        ...
                        return reject(errors.InternalError.customizeDescription(
                            `Error occurred when making request, ${body.message}`
                        ));
                    }
                    return resolve(body);
                });
            ...
            req.on('error', err => {
                this._logger.error('Error occurred when making request', {
                    method: 'KafkaConnectManager.makeRequest',
                    error: err.message,
                    params,
                });
                reject(errors.InternalError.customizeDescription(
                    `Error occurred when making request, ${err.message}`));
            });

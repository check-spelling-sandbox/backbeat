...
class ChangeStream {
    ...
    async _handleChangeStreamErrorEvent() {
        this._logger.error('An error occurred when listening to the change stream', {
        ...
        if (this._throwOnError) {
            throw errors.InternalError.customizeDescription('An error occurred while reading the change stream');
        }
        // resetting change stream
        this.start();
    }

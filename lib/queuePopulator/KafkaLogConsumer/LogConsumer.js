...
class LogConsumer {
    ...
    _waitForAssignment(wait, cb) {
        setTimeout(() => {
            // assignments contain the partitions
    ...
    /**
     * Queries last committed partition offsets
     * and stores them
     * @param {Function} cb callback
     * @returns {undefined}
     */
    ...
    _consumeKafkaMessages(limit, cb) {
        ...
        this._consumer.consume(limit, (err, messages) => {
            if (err) {
                this._log.error('An error occurred while consuming messages', {
